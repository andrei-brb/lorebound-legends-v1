...
export default function TradeUI({ playerState, onStateChange }: TradeUIProps) {
  ...
  const toggleRequested = (id: string) => { setRequestedCards(prev => prev.includes(id) ? prev.filter(x => x !== id) : prev.length < 3 ? [...prev, id] : prev); };
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Card className="bg-card/60 border-border">
              ...
              <CardContent className="space-y-3">
                <Input value={friendSearch} onChange={(e) => setFriendSearch(e.target.value)} placeholder="Search by username or Discord ID..." />
                <button
                  onClick={async () => {
