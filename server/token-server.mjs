...
/**
 * Public deck view: any authenticated player can view any other player's decks.
 * Returns both DB decks and local `deckPresets` (stored on Player JSON).
 */
async function handleGetUserDecks(req, res, playerId) {
  const user = await requireAuth(req, res);
  if (!user) return;

  const targetId = Number(playerId);
  if (!Number.isFinite(targetId)) return sendJson(res, 400, { error: "Invalid player id" });

  const target = await prisma.player.findUnique({ where: { id: targetId } });
  if (!target) return sendJson(res, 404, { error: "Player not found" });

  const decks = await prisma.deck.findMany({ where: { playerId: target.id } });
  return sendJson(res, 200, {
    ok: true,
    player: toPublicPlayer(target),
    decks,
    deckPresets: Array.isArray(target.deckPresets) ? target.deckPresets : [],
  });
}
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    // /api/users/:id/decks (public deck view)
    const userDecksMatch = path.match(/^\/api\/users\/(\d+)\/decks$/);
    if (userDecksMatch && method === "GET") return await handleGetUserDecks(req, res, userDecksMatch[1]);
